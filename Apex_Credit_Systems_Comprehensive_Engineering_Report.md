# Apex Credit Systems — Comprehensive Engineering & Architecture Report
...
* **Mechanism:** Calculates Debt-to-Income (DTI) ratio and Loan-to-Income (LTI) ratio:
  $$	ext{Monthly Income} = rac{	ext{ApplicantIncome} + 	ext{CoapplicantIncome}}{12}$$
  $$	ext{DTI Ratio} = rac{	ext{Estimated Monthly Installment (EMI)}}{	ext{Monthly Income}}$$
...
$$L_{	ext{counter}} = \min\left(L_{	ext{requested}}, \, L_{	ext{haircut}}ight)$$
